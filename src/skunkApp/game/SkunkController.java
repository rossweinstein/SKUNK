...
public class SkunkController {
	...
	public SkunkController(int numOfPlayers, String[] users) {
	...
	/**
	 * This is for the sudden death round when after the final round we have two
	 * players with the same high score. This finds those players so they can
	 * battle to win the round.
	 * 
	 * @param players
	 *            SkunkPlayer the current SkunkPlayers
	 * @return List the top players in the group
	 */
	public List<SkunkPlayer> getTopPlayers(List<SkunkPlayer> players) {

		int highScore = this.sortPlayersInDesceningOrder(players).get(0).getScore();
		return players.stream().filter(player -> player.getScore() == highScore).collect(Collectors.toList());
	}
}
