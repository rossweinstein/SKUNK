...
/**
 * 
 * SkunkController brings everything together to play skunk. It handles all the
 * input for the game and ensures the proper functionality for the Skunk Class.
 * 
 * @author Ross Weinstein
 *
 */

public class SkunkAppController {

	private SkunkController skunkGame;
	private InputHelper input;
	private boolean finalRound;
	private boolean suddenDeathRound;

	public SkunkAppController(int numOfPlayers, String[] userNames) {
		this.skunkGame = new SkunkController(numOfPlayers, userNames);
	...
	public void playSkunk() {
		...
		while (wantsToPlayAnotherRound) {
			...
			if (this.theyWantToPlayMoreSkunk()
				...
				// make sure special rounds are both false
				this.finalRound = false;
				this.suddenDeathRound = false;
	...
	private void playRound() {
		...
		if (this.skunkGame.getTopPlayers(this.skunkGame.getPlayers()).size() != 1) {
			this.playSuddenDeathRound();
		}
	 ...
	 * they do roll a skunk, the proper penalty will be applied and their turn
	 * is over.
	 * 
	 * @param activePlayer
	...
	private void turn(SkunkPlayer activePlayer) {
		...
		while (stillWantsToRoll) {
			...
			if (this.skunkGame.playerRolledAnySkunk(currentRollValue)) {
				...
				// kitty only applies if we are not in sudden death round
	 ...
	 * ////////////////////////////////////////////
	 * 
	 * SUDDEN DEATH ROUND METHDOS
	 * 
	 *////////////////////////////////////////////

	/**
	 * Display sudden death banner and find the top players to play the round
	 */
	private void playSuddenDeathRound() {
		...
		List<SkunkPlayer> topPlayers = this.skunkGame.getTopPlayers(this.skunkGame.getPlayers());

		while (topPlayers.size() != 1) {
			topPlayers = this.suddenDeath(topPlayers);
		}
	}

	/**
	 * Play a sudden death round where the top players get one turn to see who
	 * has the most points. If they have the same number of points after one
	 * round, they will play another.
	 * 
	 * @param topPlayers
	 *            SkunkPlayer the players with equal high scores
	 * @return SkunkPlayer the players with the top scores
	 */
	private List<SkunkPlayer> suddenDeath(List<SkunkPlayer> topPlayers) {

		topPlayers.stream().forEach(player -> this.turn(player));

		return this.skunkGame.getTopPlayers(topPlayers);
	}
	...
	private void displayWinnerMessage(SkunkPlayer gameWinner) {
		System.out.println("\nCongratulations " + gameWinner.getName() + "! You won the round and collected "
				+ this.skunkGame.getKitty().getAmount() + " chips from the kitty! You now have " + gameWinner.getChips()
				+ " chips!");
	}
	...
	/**
	 * print to the console the player with the most chips once all rounds are
	 * played
	 */
	private void displayGameWinner() {
		...
		System.out.println("\nCongratulations " + gameWinner.getName() + "! You won the game with "
				+ gameWinner.getChips() + " chips!");
